...
```{r}
#Read data
df = read.csv('../data/grade_preproc.csv')
df = df %>% 
...
xtrain = train %>% select(-G3) 
xtest = test %>% select(-G3) 
ytrain = train %>% select(G3) %>% data.matrix()
ytest = test %>% select(G3)  %>% data.matrix()
```
...
## Random Forest
set.seed(0)
rf = randomForest(G3 ~ ., data = train, ntree=300, nodesize=10)
...
train = one_hot(data.table::as.data.table(train))
train = train %>% # make binary factors numeric (easier interpretation)
  mutate_at(vars(ends_with("_yes"), ends_with("_no")), funs(as.numeric))

# Fit model 
rf = randomForest(G3 ~ ., data = train, ntree=300, nodesize=10)

# prepare model for viz & calc shap
unified_model = randomForest.unify(rf, data.matrix(select(train, -G3)))
treeshap = treeshap(unified_model, data.matrix(select(train, -G3))) # may take some while
...
plot_contribution(treeshap, obs= 4)+
