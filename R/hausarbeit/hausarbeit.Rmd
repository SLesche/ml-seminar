...
subtitle: "Bang - Bang: Who Can Beat the Astros?"
author: "Sven Lesche - [GitHub](https://github.com/SLesche/ml-seminar/tree/main/R/hausarbeit)"
date: "`r Sys.Date()`"
output: 
  html_document:
    theme: flatly
...
knitr::opts_chunk$set(
  message = FALSE,
  warning = FALSE,
  echo = TRUE      # Show code by default (set to FALSE to hide code)
  # results = 'hide'  # Hide results by default (remove this line to show results)
)
```
...
```{r pitch-stats}
plot_pitch_movement(clean_data %>% filter(game_year > 2017))

plot_pitch_velocity(clean_data %>% filter(game_year > 2017))

plot_pitch_by_count(clean_data %>% filter(game_year > 2017))

plot_pitch_by_batter(clean_data %>% filter(game_year > 2017))
```
...
Predicting fastballs or off-speed pitches is also more historically accurate (looking at you... Astros). For all models, we will make use of the validation data to tune some of the hyperparameters (e.g. `nodesize` or `mtry` for random forest models, `k` for KNN models, $\lambda$ for regularized regression models etc.).


### The null model
```{r take-the-best}
always_fastball <- ml_data %>% pull(is_fastball) %>% mean()
...
```
This is a model that simply predicts the most common type of pitch thrown in a specific scenario everytime. For Max Scherzer, this is a fastball in most, but not all, scenarios. The accuracy of this model is `r take_the_best$accuracy %>% print_percent()`. If you only predicted fastball, the accuracy is `r always_fastball %>% print_percent()`.
...
### Lasso
Lasso regression (Least Absolute Shrinkage and Selection Operator) is a type of linear regression that includes a penalty to encourage sparsity in the model coefficients. This means it can shrink some coefficients to exactly zero, effectively performing feature selection and reducing model complexity.

```{r lasso, cache = TRUE}
lasso_result <- run_lasso_regression(training_data, testing_data, validation_data)
coefs <- coef(lasso_result$model, lasso_result$best_param)
```
The accuracy in the testing data is `r lasso_result$test_acc %>% print_percent()`.

### Ridge
Ridge regression is a type of linear regression that includes a penalty to prevent overfitting by shrinking the coefficients towards zero. Unlike Lasso, it does not set coefficients exactly to zero, allowing it to handle multicollinearity by distributing the impact across all features.

```{r ridge, cache = TRUE}
ridge_result <- run_ridge_regression(training_data, testing_data, validation_data)
```
The accuracy in the testing data is `r ridge_result$test_acc %>% print_percent()`.

### Random Forest
This is an ensemble learning method that constructs multiple decision trees during training and outputs the average prediction (regression) or the majority vote (classification) of the individual trees. It improves accuracy and reduces overfitting by averaging the results of a large number of trees, each trained on random subsets of the data and features. Because we are predicting pitch types, this will be a classification model.

```{r random-forest, cache = TRUE}
rf_result <- run_random_forest(training_data, testing_data, validation_data)
```

The accuracy in the testing data is `r rf_result$test_acc %>% print_percent()`. The large discrepancy between training and testing accuracy (`r rf_result$train_acc %>% print_percent()` training vs. `r rf_result$test_acc %>% print_percent()` testing) suggests that the model is severly overfitting to the training set. Therefore, I will attempt to tune the `mtry` parameter.

```{r random-forest-tune, cache = TRUE}
rf_result_tuned <- run_random_forest_tune_mtry(training_data, testing_data, validation_data)
```

After tuning, the optimal `mtry` is `r rf_result_tuned$best_param` and leads to an accuracy of `r rf_result_tuned$test_acc %>% print_percent()`.

#### Visualizing feature importance
Random Forest Models make visualizing the importance of features quite simple. I chose to visualize the importance of each feature using permutation-based importance.
...
### K-Nearest-Neighbor
k-Nearest Neighbors (k-NN) is a simple, non-parametric algorithm used for classification and regression. It works by finding the k closest training examples to a given input and predicting the output based on the majority class (for classification) or the average (for regression) of these neighbors.

```{r knn, cache = TRUE}
knn_result <- run_knn(training_data, testing_data, validation_data)
```

The accuracy in the testing data is `r knn_result$test_acc %>% print_percent()`.


### Support Vector Machines
Support Vector Machines (SVM) is a supervised learning algorithm used for classification and regression tasks. It works by finding the hyperplane that best separates data points into different classes while maximizing the margin between classes. SVMs can handle both linearly separable and non-linearly separable data by using different kernel functions to map input data into higher-dimensional feature spaces. I will first try non-linear kernel functions with polynomials of varying degrees and then fit a linear kernel with varying amounts of cost, optimizing for performance in the validation set.

```{r svm, cache = TRUE}
svm_result <- run_svm(training_data, testing_data, validation_data)
```
The accuracy in the testing data is `r svm_result$test_acc %>% print_percent()`.

```{r svm-linear, cache = TRUE}
svm_linear_result <- run_svm_linear(training_data, testing_data, validation_data)
```
The accuracy in the testing data is `r svm_linear_result$test_acc %>% print_percent()`.

### Neural Network
This neural network is a deep sequential model with multiple dense layers, utilizing rectified linear unit (ReLU) activation functions for non-linearity. It incorporates dropout regularization after each dense layer to prevent overfitting, and it's optimized using RMSprop with early stopping and learning rate adjustment based on plateauing validation performance.

```{r nn, cache = TRUE}
nn_result <- run_nn(training_data, testing_data, validation_data)
```

The accuracy in the testing data is `r nn_result$test_acc %>% print_percent()`.

```{r nn-viz, cache = TRUE}
plot(nn_result$history)
```

As you can see from the progress plot, this model with multiple dense layers severly overfits to the training data, even after adding dropout regularization and adjustments of the learning rate.

I therefore tried to fit a model with more limited layers:
```{r nn-small, cache = TRUE}
nn_result_small <- run_nn_small(training_data, testing_data, validation_data)
```

```{r nn-small-viz, cache = TRUE}
plot(nn_result_small$history)
```

The accuracy in the testing data is `r nn_result_small$test_acc %>% print_percent()`.

## The winner is...
The random forest model slightly outperforms the ridge regression to take the top spot in our competition. It can correctly predict whether the coming pitch is a fastball or not in `r rf_result$test_acc %>% print_percent()`. Further tuning of hyperparameters is necessary as the tuning of `mtry` did not improve the accuracy in the test set.

# Conclusion
The random forest model did pretty good, outperforming just picking the most frequent type in each scenario by about 3 percentage points. Is this improvement good enough to beat the Astros? Time will tell.

In my opinion, it is great to see that even sophisticated machine learning models do not do that much better than just predicting future pitches based on past pitch types. I guess the last 35% of coming pitches will remain a mystery and keep the game interesting for years to come.
