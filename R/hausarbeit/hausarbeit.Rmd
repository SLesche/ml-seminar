...
```
The accuracy in the testing data is `r rf_result$test_acc %>% print_percent()`. The large discrepance between training and testing accuracy (`r rf_result$train_acc %>% print_percent()` training vs. `r rf_result$test_acc %>% print_percent()` testing) suggests that the model is severly overfitting to the training set. Therefore, I will attempt to tune the `mtry` parameter.
